...
To fix the problem, "
    ...
    "use the -s option or "
#endif
    "set the environment variable " SUBREAPER_ENV_VAR
...
int spawn(const signal_configuration_t *const sigconf_ptr, char *const argv[],
  ...
  // TODO: check if tini was a foreground process to begin with (it's not OK to
  ...
  if (pid < 0) {
    ...
    // Put the child in a process group and make it the foreground process if
    // there is a tty.
    if (isolate_child()) {
      return 1;
    }
...
void print_usage(char *const name, FILE *const file) {
...
#endif
  fprintf(file,
          "Execute a program under the supervision of a valid init process "
          "(%s)\n\n",
          basename(name));
  ...
  fprintf(file, "  -e EXIT_CODE: Remap EXIT_CODE (from 0 to 255) to 0 (can be "
                "repeated).\n");
...
// Parses and replaces an environment variable within an argument.
void parse_and_replace_with_env_var(char buf[], char *const arg) {
  int start = -1;
  int end = -1;
  char dollar[] = "$";
  char lbrace[] = "{";
  ...
  for (i = 0; i <= strlen(arg); i++) {
    ...
    if (start != -1 && arg[i] == *rbrace) {
      end = i + 1;
    }
  }

  if (start != -1 && end != -1) {
    // Get environment variable name from the path.
    strcatslice(env, arg, start, end, 4096);
  }

  char *value = getenv(env);
  if (value != NULL) {
    if (buf[strlen(buf) - 1] == '/' && value[0] == '/') {
      // Remove first slash from environment variable because the buffer already
      // has one at the end.
...
int register_subreaper() {
  if (subreaper > 0) {
    if (prctl(PR_SET_CHILD_SUBREAPER, 1)) {
      if (errno == EINVAL) {
        PRINT_FATAL("PR_SET_CHILD_SUBREAPER is unavailable on this platform. "
                    "Are you using Linux >= 3.4?")
      } else {
        PRINT_FATAL("Failed to register as child subreaper: %s",
                    strerror(errno))
      }
      return 1;
    } else {
...
int configure_signals(sigset_t *const parent_sigset_ptr,
  ...
  // Handle SIGTTIN and SIGTTOU separately. Since Tini makes the child process
  // group the foreground process group, there's a chance Tini can end up not
  // controlling the tty. If TOSTOP is set on the tty, this could block Tini on
  // writing debug messages. We don't want that. Ignore those signals.
    ...
    default:
      ...
      if (kill(kill_process_group ? -child_pid : child_pid, sig.si_signo)) {
        if (errno == ESRCH) {
          ...
          PRINT_FATAL("Unexpected error when forwarding signal: '%s'",
                      strerror(errno));
          return 1;
        }
      }
...
int reap_zombies(const pid_t child_pid, int *const child_exitcode_ptr) {
  ...
  while (1) {
    ...
    default:
       ...
       * set the exit_code, which will cause us to exit once we've reaped
       * everyone else.
       */
      ...
      if (current_pid == child_pid) {
        if (WIFEXITED(current_status)) {
          ...
          PRINT_INFO("Main child exited normally (with status '%i')",
          ...
          *child_exitcode_ptr = WEXITSTATUS(current_status);
           ...
           */
          PRINT_INFO("Main child exited with signal (with signal '%s')",
                     strsignal(WTERMSIG(current_status)));
        ...
        *child_exitcode_ptr =
            *child_exitcode_ptr % (STATUS_MAX - STATUS_MIN + 1);
    ...
    /* If we make it here, that's because we did not continue in the switch
     * case. */
    break;
  }
...
int main(int argc, char *argv[]) {
  ...
  int child_exitcode = -1; // This isn't a valid exitcode, and lets us tell
